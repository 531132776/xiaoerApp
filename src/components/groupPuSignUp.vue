  ...
  <div class="groupPuSignUp common_news">
    ...
    <section class="groupPuSP_midd bg_fff">
          ...
          <mu-list-item  button :ripple="false">
            ...
            <mu-list-item-content>
              ...
              <mu-list-item-sub-title>
                <span class="text-999">当前有<em class="text-red">{{CurrentGroupBuyingInformation.intentionerNumber}}</em>人报名</span>
              </mu-list-item-sub-title>
            </mu-list-item-content>
          </mu-list-item>
        </mu-list>
      ...
      <div class="groupbrand_img">
        <mu-container fluid>
            <img :src="CurrentGroupBuyingInformation.titleImgage" class="img-responsive" alt="">
        </mu-container>
        <mu-list>
          <mu-list-item>
            <mu-list-item-content>
              <mu-list-item-sub-title>时间：<span>{{CurrentGroupBuyingInformation.startTime}} - {{CurrentGroupBuyingInformation.endTime}}</span></mu-list-item-sub-title>
              <mu-list-item-sub-title>地点：<span>{{CurrentGroupBuyingInformation.address}}</span> </mu-list-item-sub-title>
    ...
    <div class="bottom_price pt10">
      <div class="bottom_price_lists bg_fff">
        <div class="lg_title">
          <div class="mu-sub-header">团购车型</div>
        </div>
        <mu-divider></mu-divider>
        <div class="price_list_content pb10">
          <div class="price_list_item p15 view_cell_after after_last" v-for="(item,index) in CarMoneyList" :key="index">
            <div class="prcie_item_left">
              <div class="carMoney_model">{{item.name}}</div>
              <span class="text-999 text_line_through text_14">{{item.mPrice}}</span>
            </div>
            <div class="prcie_item_right">
              ...
              <div>
                <mu-button color="secondary" small @click="openFloorprice(item.seriesId,item.manufacturer)">询问底价</mu-button>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  data () {
    return {
      ...
      custom: {
        ...
        CurrentGroupBuyingInformation:{}
    }
  },
  
  created () {
    // this.$store.state.showTopNav = false;//当辞页面是子页面时才需要
    // let _relus = this.$route.params.content;
    // let _params = JSON.parse(_relus);
    // console.log(this.$route.params.id)
    // console.log(_params);
    // this.CurrentGroupBuyingInformation = _params;
    this.groupCarId = this.$store.state.groupSignup.id || JSON.parse(sessionStorage.getItem('tuangou')).id;
    this.CurrentGroupBuyingInformation = this.$store.state.groupSignup.item || JSON.parse(sessionStorage.getItem('tuangou')).item;
    //获取城市
    this.getCityList();
    //获取最新团购团友
  ...
  methods:{
    ...
    //定时获取车款
    // setTimeoutChoiceCar(){
    //   apiAll.getCarSeries({'seriesId':this.groupPurchSignUpInfo.carId}).then(res => {
    //     console.log(res)
    //     if(res.data != null && res.data !=undefined){
    //       this.CarMoneyList = res.data;
    //     }
    ...
    getCarBrand(){
      ...
      // try{
        // let getCar = await 
        apiAll.getCarBrand({"id":this.groupCarId}).then(getCar => {
        console.log(getCar)
